...
string TrainerName(string k) {
  ...
  if (hour < 12) {
    ...
    userInteractMessage.setString("Welcome back!\nAre you excited?\nPress confirm to go on mate!");
      ...
      switch (event.type) {
        ...
        case sf::Event::MouseButtonPressed:
          if (confirmButton.isMouseOver(window)) {
            ...
            ifstream inFile;
            inFile.open("tName.txt");
            if (!inFile||k=="y") {
              if (trainerName != "") {
                std::cout << "CONFIRM\n";
                cout << "Trainer's name is: " << trainerName << endl;
                ofstream inFile("tName.txt");
                inFile << trainerName;
                inFile.close();
                window.close();
