...
void BattleBlaVsGren(string trainerName) {
  // Creating a Blastoise
  Blastoise *Bl = new Blastoise;
  
  // Creating enemy geninja with health of 8, element of water and speed of 3

  Pokemon *Gen = new Pokemon(8, 1, 3, "Geninja", 0);
  // SFML RenderWindow
  ...
  sf::Texture blastoiseB;
  blastoiseB.loadFromFile("Resources/blastoise-b.png");
  sf::Sprite blastoiseBSprite;
  blastoiseBSprite.setTexture(blastoiseB);
  blastoiseBSprite.setPosition(sf::Vector2f(20, 215));
  blastoiseBSprite.scale(sf::Vector2f(2.6, 2.6));
  ...
  // Make buttons
  Button m1(Bl->M1.getMoveName(), {120, 40}, 16, sf::Color::Blue,
            sf::Color::Black);
  m1.setFont(courierNew);
  m1.setPosition({435, 420});
  Button m2(Bl->M2.getMoveName(), {120, 40}, 16, sf::Color::Blue,
            sf::Color::Black);
  m2.setFont(courierNew);
  m2.setPosition({565, 420});
  Button m3(Bl->M3.getMoveName(), {120, 40}, 16, sf::Color::Blue,
            sf::Color::Black);
  m3.setFont(courierNew);
  m3.setPosition({435, 470});
  Button m4(Bl->M4.getMoveName(), {120, 40}, 16, sf::Color::Blue,
            sf::Color::Black);
  ...
  statusA.setString("Blastoise  Element: Water\nHP: 10\tSpeed: 7");
  ...
  while (window.isOpen()) {
    ...
    while (window.pollEvent(event)) {
      switch (event.type) {
        ...
        case sf::Event::MouseButtonPressed:
          if (m1.isMouseOver(window)) {
            if (skillAvailable == 1) {
              hea = battleW(skillAvailable, round, *Bl, *Gen, 1,
                      trainerName,hea);
            ...
            if (skillAvailable == 1) {
              hea = battleW(skillAvailable, round, *Bl, *Gen, 2,
                      trainerName,hea);
            } else if (skillAvailable == 0) {
              hea = battleW(skillAvailable, round, *Bl, *Gen, 2,
                      trainerName,hea);
            ...
            if (skillAvailable == 1) {
              battleW(skillAvailable, round, *Bl, *Gen, 3,
                      trainerName,hea);
              round++;
              skillAvailable = 0;
            }
            ...
            if (skillAvailable == 1) {
              hea = battleW(skillAvailable, round,*Bl, *Gen, 4,
                      trainerName,hea);
            } else if (skillAvailable == 0) {
              hea = battleW(skillAvailable, round,*Bl, *Gen, 4,
                      trainerName,hea);
        ...
        case sf::Event::MouseMoved:
          ...
          if (m3.isMouseOver(window)) {
            if (skillAvailable == 1) {
              userInteractMessage.setString(currentMessage);
              m3.setBackColor(sf::Color::Blue);
            } else if (skillAvailable == 0) {
              userInteractMessage.setString(
                  "Trainer, Blastoise does not have\nHydro Pump skill shot "
                  "left.");
      ...
      } else if (Gen->getHealth() <= 0) {
        window.close();
        delete Gen;
        delete Bl;
        Result(1, 'w', trainerName);
    ...
    if (Gen->getHealth() < (8 * 0.5)) {
      hpBarB.setFillColor(sf::Color::Yellow);
    }
    if (Gen->getHealth() < (8 * 0.2)) {
      hpBarB.setFillColor(sf::Color::Red);
    }
    hpBarA.setSize(sf::Vector2f(hea * 21.5, 20));
    hpBarB.setSize(sf::Vector2f(Gen->getHealth() * 27, 20));
    window.draw(hpBarA);
    window.draw(hpBarB);
