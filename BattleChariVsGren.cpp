...
void BattleChariVsGren(string trainerName) {
  ...
  sf::Texture charizardB;
  charizardB.loadFromFile("Resources/charizard-b.png");
  sf::Sprite charizardBSprite;
  charizardBSprite.setTexture(charizardB);
  charizardBSprite.setPosition(sf::Vector2f(20, 200));
  charizardBSprite.scale(sf::Vector2f(2.4, 2.4));
  ...
  // Make buttons
  Button m1(Ch->M1.getMoveName(), {120, 40}, 16, sf::Color::Red,
            sf::Color::Black);
  m1.setFont(courierNew);
  m1.setPosition({435, 420});
  Button m2(Ch->M2.getMoveName(), {120, 40}, 16, sf::Color::Red,
            sf::Color::Black);
  m2.setFont(courierNew);
  m2.setPosition({565, 420});
  Button m3(Ch->M3.getMoveName(), {120, 40}, 16, sf::Color::Red,
            sf::Color::Black);
  m3.setFont(courierNew);
  m3.setPosition({435, 470});
  Button m4(Ch->M4.getMoveName(), {120, 40}, 16, sf::Color::Red,
            sf::Color::Black);
  ...
  statusA.setString("Charizard  Element: Fire\nHP: 10\tSpeed: 7");
      ...
      switch (event.type) {
        ...
        case sf::Event::MouseButtonPressed:
          if (m1.isMouseOver(window)) {
            if (skillAvailable == 1) {
              hea = battleF(skillAvailable, round, *Ch, *Gen, 1,
                      trainerName,hea);
            ...
            if (skillAvailable == 1) {
              hea = battleF(skillAvailable, round, *Ch, *Gen, 2,
                      trainerName,hea);
            } else if (skillAvailable == 0) {
              hea = battleF(skillAvailable, round, *Ch, *Gen, 2,
                      trainerName,hea);
            ...
            if (skillAvailable == 1) {
              hea = battleF(skillAvailable, round, *Ch, *Gen, 3,
                      trainerName,hea);
              round++;
              skillAvailable = 0;
            }
            ...
            if (skillAvailable == 1) {
              hea = battleF(skillAvailable, round, *Ch, *Gen, 4,
                      trainerName,hea);
            } else if (skillAvailable == 0) {
              hea = battleF(skillAvailable, round, *Ch, *Gen, 4,
                      trainerName,hea);
        ...
        case sf::Event::MouseMoved:
          ...
          if (m3.isMouseOver(window)) {
            if (skillAvailable == 1) {
              userInteractMessage.setString(currentMessage);
              m3.setBackColor(sf::Color::Red);
            } else if (skillAvailable == 0) {
              userInteractMessage.setString(
                  "Trainer, Charizard does not have\nFire Burst skill shot "
                  "left.");
