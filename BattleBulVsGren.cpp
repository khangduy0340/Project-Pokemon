...
void BattleBulVsGren(string trainerName) {
  ...
  // Make buttons
  Button m1(Bu->M1.getMoveName(), {120, 40}, 16, sf::Color::Green,
            sf::Color::Black);
  m1.setFont(courierNew);
  m1.setPosition({435, 420});
  Button m2(Bu->M2.getMoveName(), {120, 40}, 16, sf::Color::Green,
            sf::Color::Black);
  m2.setFont(courierNew);
  m2.setPosition({565, 420});
  Button m3(Bu->M3.getMoveName(), {120, 40}, 16, sf::Color::Green,
            sf::Color::Black);
  m3.setFont(courierNew);
  m3.setPosition({435, 470});
  Button m4(Bu->M4.getMoveName(), {120, 40}, 16, sf::Color::Green,
            sf::Color::Black);
  ...
  int round = 1;
  int hea = Bu->getHealth();
  // Window Loop
  while (window.isOpen()) {
    // int hea = Bulbasaur.getHealth();
    sf::Event event;
    // Event Poll Loop
      ...
      switch (event.type) {
        ...
        case sf::Event::MouseButtonPressed:
          if (m1.isMouseOver(window)) {
            if (skillAvailable == 1) {
              hea = battleG(skillAvailable, round, *Bu, *Gen, 1,
                      trainerName,hea);
                      cout << hea<<endl;
            ...
            if (skillAvailable == 1) {
              hea = battleG(skillAvailable, round, *Bu, *Gen, 2,
                      trainerName,hea);
            } else if (skillAvailable == 0) {
              hea = battleG(skillAvailable, round, *Bu, *Gen, 2,
                      trainerName,hea);
            ...
            if (skillAvailable == 1) {
              hea = battleG(skillAvailable, round, *Bu, *Gen, 3,
                      trainerName,hea);
              round++;
              skillAvailable = 0;
            }
            ...
            if (skillAvailable == 1) {
              hea = battleG(skillAvailable, round, *Bu, *Gen, 4,
                      trainerName,hea);
            } else if (skillAvailable == 0) {
              hea = battleG(skillAvailable, round, *Bu, *Gen, 4,
                      trainerName,hea);
